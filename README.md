...
### Приклади використання
  ...
  * **Сканування через Tor/ProxyChains для Nuclei**:

    ```bash
    python3 scan_nuclei_dd.py --proxyTor example.com
    ```

    (Усі запуски Nuclei будуть виконуватися через ProxyChains незалежно від доступності ресурсів напряму.)
...
### Перевірка доступності та ProxyChains
  ...
  * За потреби можна примусово запускати Nuclei через `proxychains`, додавши аргумент `--proxyTor`.
...
Якщо додати цей аргумент, **усі** запуски Nuclei виконуватимуться через `proxychains`, минаючи попередню перевірку доступності напряму. Це корисно, коли необхідно приховати реальну IP-адресу або сканувати виключно через мережу Tor/проксі.
